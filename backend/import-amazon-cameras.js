/**
 * Import kamera asli dari dataset Amazon 2023 (amazon-cameras.json)
 * + unduh foto asli dari CDN Amazon ke MinIO.
 * Usage (dalam container backend): node import-amazon-cameras.js
 */
...
async function run() {
  try {
    const data = JSON.parse(fs.readFileSync(`${__dirname}/amazon-cameras.json`, 'utf8'));
    await mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/elektroniku');
    console.log(`✅ MongoDB terhubung. ${data.length} kamera di dataset.`);

    const cat = await Category.findOne({ slug: 'kamera' });
    if (!cat) throw new Error("Kategori 'kamera' tidak ditemukan");
    ...
    console.log(`🆕 ${fresh.length} kamera baru (${data.length - fresh.length} sudah ada, dilewati).`);
    ...
    for (const d of fresh) {
      ...
      for (let i = 0; i < Math.min(d.images.length, 3); i++) {
        ...
        if (img) {
          ...
          const objName = `cam-${d.asin}-${i}.${ext}`;
    ...
    console.log(`\n✅ Selesai! ${created} kamera diimport, ${noImg} dilewati (gambar gagal). Total: ${total} produk.`);
  } catch (err) {

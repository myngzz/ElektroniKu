/**
 * Import headphone/earphone asli dari dataset Amazon 2023 (amazon-headphones.json)
 * + unduh foto asli dari CDN Amazon ke MinIO.
 * Usage (dalam container backend): node import-amazon-headphones.js
 */
...
async function run() {
  try {
    const data = JSON.parse(fs.readFileSync(`${__dirname}/amazon-headphones.json`, 'utf8'));
    await mongoose.connect(process.env.MONGO_URI || 'mongodb://localhost:27017/elektroniku');
    console.log(`✅ MongoDB terhubung. ${data.length} headphone di dataset.`);

    const cat = await Category.findOne({ slug: 'headphone' });
    if (!cat) throw new Error("Kategori 'headphone' tidak ditemukan");
    ...
    console.log(`🆕 ${fresh.length} headphone baru (${data.length - fresh.length} sudah ada, dilewati).`);
    ...
    for (const d of fresh) {
      ...
      for (let i = 0; i < Math.min(d.images.length, 2); i++) {
        ...
        if (img) {
          ...
          const objName = `amz-${d.asin}-${i}.${ext}`;
      ...
      await Product.create({
        ...
        isFeatured: d.rating_count > 50000 && Math.random() < 0.3,
    ...
    console.log(`\n✅ Selesai! ${created} headphone diimport, ${noImg} dilewati (gambar gagal). Total: ${total} produk.`);
  } catch (err) {

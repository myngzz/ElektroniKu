...
function CompareContent() {
 ...
 <p className="text-sm text-gray-500 mt-1">Pilih 2-3 produk dan biarkan AI menganalisis perbandingannya</p>
 </div>
 ...
 return (
 <div key={product._id} className="bg-white rounded-2xl border border-gray-100 p-4 relative">
 <button
 onClick={() => removeProduct(product._id)}
 className="absolute top-3 right-3 w-6 h-6 bg-red-100 text-red-500 rounded-full flex items-center justify-center hover:bg-red-200"
 >
 <X className="w-3 h-3" />
 </button>
 ...
 <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 p-4 flex flex-col items-center justify-center gap-2 min-h-[200px]">
 ...
 <th className="text-left px-4 py-3 text-gray-600 font-medium w-1/4">Spesifikasi</th>
 {selectedProducts.map((p) => (
 <th key={p._id} className="text-left px-4 py-3 text-gray-900 font-medium">
 ...
 <td className="px-4 py-3 text-gray-600 font-medium">Harga</td>
 {selectedProducts.map((p) => (
 <td key={p._id} className="px-4 py-3 font-bold text-gray-900">{formatPrice(p.price)}</td>
 ))}
 </tr>
 <tr className="border-t border-gray-100 bg-gray-50">
 <td className="px-4 py-3 text-gray-600 font-medium">Rating</td>
 {selectedProducts.map((p) => (
 <td key={p._id} className="px-4 py-3 text-gray-900">{p.avgRating}/5 ⭐</td>
 ))}
 </tr>
 {allSpecKeys.map((key, idx) => (
 <tr key={key} className={`border-t border-gray-100 ${idx % 2 === 0 ?'' :'bg-gray-50'}`}>
 <td className="px-4 py-3 text-gray-600 font-medium capitalize">{key.replace(/_/g,'')}</td>
 {selectedProducts.map((p) => {
 const specs = p.specifications instanceof Map
 ...
 const val = specs[key];
 return (
 <td key={p._id} className="px-4 py-3 text-gray-900">
 ...
 <h2 className="font-semibold text-gray-900 mb-3">Tambah Produk untuk Dibandingkan</h2>
 <input
 type="text"
 placeholder="Cari produk..."
 value={searchQuery}
 onChange={(e) => setSearchQuery(e.target.value)}
 className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 mb-3 bg-gray-50"
 />
 ...
 className="text-left p-2 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors"
 >
 <p className="text-xs font-medium text-gray-800 line-clamp-2 mb-0.5">{p.name}</p>
 <p className="text-xs text-gray-900">{formatPrice(p.price)}</p>
 </button>
 ))}
 </div>
 </div>

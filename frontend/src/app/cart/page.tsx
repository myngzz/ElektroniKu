...
export default function CartPage() {
 ...
 {items.map((item) => (
 <div key={item.product._id} className="bg-white rounded-2xl border border-gray-100 p-4 flex gap-4">
 ...
 <Link href={`/catalog/${item.product._id}`} className="font-medium text-gray-900 hover:text-gray-900 line-clamp-2 text-sm block mb-1">
 {item.product.name}
 </Link>
 <p className="text-xs text-gray-500 mb-2">{item.product.brand}</p>
 <p className="font-bold text-gray-900 text-sm mb-3">{formatPrice(item.product.price)}</p>
 ...
 className="p-1.5 hover:bg-gray-200 disabled:opacity-50 rounded-lg transition-colors"
 >
 ...
 <Link href="/catalog" className="block text-center text-sm text-gray-900 hover:underline mt-3">
 Lanjut Belanja
 </Link>
 </div>
 </div>

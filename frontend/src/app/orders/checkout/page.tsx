...
export default function CheckoutPage() {
 ...
 <span className="text-gray-600 line-clamp-1 flex-1">{p?.name ||'Produk'} ×{item.qty}</span>
 <span className="ml-2 text-gray-900 shrink-0">{formatPrice((p?.price || 0) * item.qty)}</span>
 </div>
 );
 })}
 </div>
